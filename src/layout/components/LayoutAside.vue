  ...
  <div ref="rootRef" class="layout-aside">
    <div class="menu-container">
      ...
      >
        ...
        <span
          class="line"
          :style="[
            { transform: 'translateY(' + state.lineOffsetTop + 'px)' },
            { visibility: state.lineOffsetTop >= 0 ? 'initial' : 'hidden' },
          ]"
        />
      </el-menu>
    </div>
    ...
    <div class="setting" @click.prevent="toggleMenu">
      <svg class="icon" aria-hidden="true" width="22px" height="20px">
        <use
          :xlink:href="`#${
            menuCollapse ? 'iconcaidanzhankai' : 'iconcaidanshouqi'
          }`"
        />
      </svg>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const asideList = computed(() => {
      const asideList =
        props.firstMenu?.children?.filter(
          (item: CurMenuType) => item.tab !== false
        ) ?? [];
      return asideList;
    });
    ...
    const activeLine = () => {
      nextTick(() => {
        ...
        if (item) {
          ...
          state.lineOffsetTop =
            offsetTop + (offsetHeight - (line as HTMLElement).offsetHeight) / 2;
        } else {
    ...
    // 跳转dashboard
    const skipDashboard = () => {
      const curModule = $store.state.basic.menuList.find((item: CurMenuType) =>
        window.location.pathname.startsWith(item.routePath)
      );
