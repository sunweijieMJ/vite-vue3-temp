  ...
  <div class="layout-header">
    ...
    <div class="menu-container">
      ...
      <div v-if="false" class="search" :class="{ empty: !keywords }">
        ...
        <el-input
          v-model="keywords"
          :placeholder="$t('baseLayoutHeader.t139')"
        />
      </div>
      <!-- user -->
      <div class="user">
        ...
        <template #dropdown>
          <el-dropdown-menu class="header-dropdown">
            <el-dropdown-item
              ...
              :key="index"
              :class="{ active: item.title === firstMenu.title }"
              :command="item"
            >
              <div class="menu-left">
                <div class="name-box">
                  <svg
                    v-if="item.title !== firstMenu.title"
                    class="icon"
                    aria-hidden="true"
                    width="24px"
                    height="24px"
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleUser = (command: string) => {
      switch (command) {
        case 'modify':
          $store.dispatch('basic/toggleModifyPass', {
            status: true,
            type: 'modify',
          });
          break;
        case 'logout':
          ElMessageBox.confirm(
            `${$i18n.t('baseLayoutHeader.t143')}?`,
            `${$i18n.t('baseLayoutHeader.t101')}`,
            {
            ...
            basicApi.authLoginOut().then((res) => {
              if (res.status) {
                if (res.data) {
                  ...
                  ElMessage({
                    type: 'error',
                    message: `${$i18n.t('baseLayoutHeader.t144')}`,
                  });
                }
              }
            });
