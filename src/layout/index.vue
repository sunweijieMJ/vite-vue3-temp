...
export default defineComponent({
  ...
  setup() {
    ...
    const firstMenu = computed<Partial<CurMenuType>>(() => {
      ...
      const firstMenu = menuList.value.find(
        (item) => item.routeName === curPathname.split('/')[1]
      );
    ...
    const secondMenu = computed<Partial<CurMenuType>>(() => {
      ...
      const secondMenu = firstMenu.value.children.find(
        (item) => item.routeName === curPathname.split('/')[2]
      );
    ...
    const initMenu = () => {
      if (!menuList.value.length) {
        ElMessageBox.alert(
          `${$i18n.t('baseLayoutIndex.t100')}`,
          `${$i18n.t('baseLayoutIndex.t101')}`,
          {
