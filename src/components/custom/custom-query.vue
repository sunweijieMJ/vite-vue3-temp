<template>
  <el-form
    ref="rootRef"
    class="custom-query"
    :style="isCollapse ? { height: minHeight } : { height: maxHeight }"
    :model="currentForm"
  >
    <div class="query-container">
      ...
      <div class="control-box">
        <div class="control">
          <div class="btn-box">
            <el-button class="reset" @click.prevent="resetForm">{{
              $t('baseCustomQuery.t1')
            }}</el-button>
            <el-button
              class="query"
              native-type="submit"
              @click.prevent="$emit('queryForm')"
              >{{ $t('baseCustomQuery.t2') }}
            </el-button>
          </div>
          <div v-if="showToggle" class="toggle" @click.prevent="queryToggle">
            <span class="text">{{
              isCollapse ? $t('baseCustomQuery.t3') : $t('baseCustomQuery.t4')
            }}</span>
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      nextTick(() => {
        ...
        maxHeight.value = `${
          rootRef.value?.$el.querySelector('.query-container')
