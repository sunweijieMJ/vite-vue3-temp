...
// 判断浏览器及终端
const os = (
  u: string = window?.navigator.userAgent
...
const adjustTime = (
  ...
  const targetTime =
    new Date(time).getTime() + diff * MINUTE_MS - targetTimezone * HOUR_MS;
...
const countDown = ({
  ...
  type,
  remain,
}: CountDownParams): Record<string, string | boolean> => {
  let delta = 0;
  // 是否为剩余时间戳
  if (remain) {
    delta = +time;
    ...
    delta =
      new Date(adjustTime(time)).getTime() - new Date(adjustTime()).getTime();
  }
  if (delta <= 0) {
    return {
      over: true,
  ...
  switch (type) {
    case 'DD:HH:MM:SS':
      ...
      // 计算秒数
      secs = Math.floor(
        (delta - days * DAY_MS - hours * HOUR_MS - mins * MINUTE_MS) / SECOND_MS
      );
      break;
    case 'HH:MM:SS':
      ...
      secs = Math.floor(
        (delta - hours * HOUR_MS - mins * MINUTE_MS) / SECOND_MS
      );
      break;
    case 'MM:SS':
...
const textTohtml = (text: string): string => {
  ...
  let html: string = text.replace(
    link,
    ($1): string => `<a class="link" href="${$1}" target="_blank">${$1}</a>`
  );
  // 匹配邮箱
  html = html.replace(
    email,
    ($1): string =>
      `<a class="email" href="mailto:${$1}" target="_blank">${$1}</a>`
  );
  // 匹配换行
  html = html.replace(/\r|\n/g, '<br>');
  return `<p>${html}</p>`;
