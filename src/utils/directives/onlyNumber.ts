...
export const onlyNumber: Directive = {
    beforeMount(el, vDir) {
        ...
        el.addEventListener('keypress', (evt: KeyboardEvent) => {
            const inputKey = String.fromCharCode(typeof evt.charCode === 'number' ? evt.charCode : evt.keyCode);
            const re = /\d|\.|-/;
        ...
        el.addEventListener('keyup', (evt: KeyboardEvent) => {
            if ((evt.target as HTMLInputElement)?.value) {
                ...
                if (!isNaN(argMax) && content > argMax) {
                    (evt.target as HTMLInputElement).value = argMax + '';
                    content = argMax;
                }
                if (!isNaN(argMin) && content < argMin) {
        ...
        // 失去焦点=>保留指定位小数
        el.addEventListener('focusout', (evt: KeyboardEvent) => {// 此处会在 el-input 的 @change 后执行
            // 限制最大最小值
            if ((evt.target as HTMLInputElement).value) {
                content = parseFloat((evt.target as HTMLInputElement).value);
                if (!content) {
